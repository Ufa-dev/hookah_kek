...
fun AuthorizedWebTestClient.createMarket(
    ...
    post().uri(MARKET_URL)
        ...
        .bodyValue(MarketCreateDto(brandId = brandId, flavorId = flavorId, name = name, weightGrams = weightGrams, gtin = gtin))
        .exchange()
...
suspend fun AuthorizedWebTestClient.createMarketAndGet(
    ...
    return createMarket(brandId = brand.id.id, flavorId = flavor.id.id, name = name, weightGrams = weightGrams)
        .expectStatus().isOk
...
fun AuthorizedWebTestClient.updateMarket(
    ...
    put().uri("$MARKET_URL/$id")
        .contentType(MediaType.APPLICATION_JSON)
        .bodyValue(MarketUpdateDto(brandId = brandId, flavorId = flavorId, name = name, weightGrams = weightGrams, gtin = gtin))
        .exchange()
